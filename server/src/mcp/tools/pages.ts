...
export const searchPagesTool = defineTool({
  name: "search_pages",
  description:
    "Search across all documentation pages for a query string. Returns matching pages with snippets and match counts.",
  ...
  handler: async (args, context) => {
    ...
    const getAllFolders = async (
      node: {
        path?: string;
        children?: Array<{ path?: string; children?: unknown[] }>;
      },
      folders: string[] = [],
    ): Promise<string[]> => {
...
export const getPageTool = defineTool({
  ...
  inputSchema: z.object({
    path: z
      .string()
      .describe('Path to the page (e.g., "folder/page.md" or "Welcome.md")'),
  }),
  annotations: {
    readOnlyHint: true,
  },
...
export const createPageTool = defineTool({
  ...
  inputSchema: z.object({
    path: z
      .string()
      .describe('Path for the new page (e.g., "folder/new-page.md")'),
  ...
  handler: async (args, context) => {
    ...
    try {
      ...
      const pagePath = args.path.endsWith(".md")
        ? args.path
        : `${args.path}.md`;
      await fileSystemService.createPage(pagePath, args.content);
      return {
        content: [
          { type: "text", text: `Successfully created page: ${pagePath}` },
        ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to create page: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
...
export const updatePageTool = defineTool({
  name: "update_page",
  description:
    "Update the content of an existing documentation page. Replaces the entire content.",
  inputSchema: z.object({
  ...
  handler: async (args, context) => {
    ...
    try {
      ...
      return {
        content: [
          { type: "text", text: `Successfully updated page: ${args.path}` },
        ],
...
export const deletePageTool = defineTool({
  name: "delete_page",
  description:
    "Delete a documentation page. This action cannot be undone (though Git history preserves it).",
  inputSchema: z.object({
  ...
  handler: async (args, context) => {
    ...
    try {
      ...
      return {
        content: [
          { type: "text", text: `Successfully deleted page: ${args.path}` },
        ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to delete page: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
...
export const renamePageTool = defineTool({
  name: "rename_page",
  description:
    "Rename a documentation page (changes filename only, not folder).",
  inputSchema: z.object({
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Successfully renamed page from ${args.oldPath} to ${args.newPath}`,
          },
        ],
        structuredContent: {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to rename page: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
...
export const movePageTool = defineTool({
  ...
  inputSchema: z.object({
    ...
    newFolder: z
      .string()
      .describe("Destination folder path (empty string for root)"),
  }),
  annotations: {
    destructiveHint: true,
  },
  handler: async (args, context) => {
    ...
    try {
      ...
      return {
        content: [
          { type: "text", text: `Successfully moved page to ${newPath}` },
        ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to move page: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
