...
export const listFoldersTool = defineTool({
  name: "list_folders",
  description:
    "Get the complete folder tree structure of the documentation. Returns all folders and their hierarchy.",
  inputSchema: z.object({
    path: z
      .string()
      .optional()
      .default("")
      .describe("Starting path (empty for root)"),
  }),
  annotations: {
    readOnlyHint: true,
  },
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to list folders: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
...
export const listPagesTool = defineTool({
  ...
  inputSchema: z.object({
    folder: z
      ...
      .optional()
      .default("")
      .describe("Folder path (empty for root)"),
  }),
  annotations: {
    readOnlyHint: true,
  },
  handler: async (args, context) => {
    ...
    try {
      ...
      if (pages.length === 0) {
        return {
          content: [
            {
              type: "text",
              text: `No pages found in ${args.folder || "root"}`,
            },
          ],
      ...
      const pageList = pages
        .map(
          (p) =>
            `📄 ${p.name} (modified: ${new Date(p.modifiedAt).toLocaleDateString()})`,
        )
        .join("\n");
      ...
      return {
        content: [
          {
            type: "text",
            text: `Pages in ${args.folder || "root"}:\n\n${pageList}`,
          },
        ],
        structuredContent: { folder: args.folder, pages },
      };
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to list pages: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
...
export const createFolderTool = defineTool({
  ...
  inputSchema: z.object({
    path: z
      .string()
      .describe('Path for the new folder (e.g., "Projects/NewProject")'),
  }),
  annotations: {
  ...
  handler: async (args, context) => {
    ...
    try {
      ...
      return {
        content: [
          { type: "text", text: `Successfully created folder: ${args.path}` },
        ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to create folder: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
...
export const deleteFolderTool = defineTool({
  ...
  handler: async (args, context) => {
    ...
    try {
      ...
      return {
        content: [
          { type: "text", text: `Successfully deleted folder: ${args.path}` },
        ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to delete folder: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Successfully renamed folder from ${args.oldPath} to ${args.newPath}`,
          },
        ],
        structuredContent: {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to rename folder: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
...
export const moveFolderTool = defineTool({
  name: "move_folder",
  description:
    "Move a folder (and all its contents) to a different location in the folder hierarchy.",
  inputSchema: z.object({
    sourcePath: z
      .string()
      .describe('Path of the folder to move (e.g. "Projects/OldLocation")'),
    destinationParentPath: z
      .string()
      .describe(
        'Path of the destination parent folder, or empty string to move to root (e.g. "Archive" or "")',
      ),
  }),
  annotations: {
    destructiveHint: true,
  ...
  handler: async (args, context) => {
    ...
    try {
      const newPath = await fileSystemService.moveFolder(
        args.sourcePath,
        args.destinationParentPath,
      );
      return {
        content: [
          {
            type: "text",
            text: `Successfully moved folder from ${args.sourcePath} to ${newPath}`,
          },
        ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to move folder: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
