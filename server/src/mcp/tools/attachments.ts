...
export const listAttachmentsTool = defineTool({
  ...
  description:
    "List all attachments in a folder. Attachments are stored in .attachments subdirectories.",
  ...
  handler: async (args, context) => {
    ...
    try {
      const attachmentsDir = validatePath(
        DATA_DIR,
        args.folder,
        ".attachments",
      );
        ...
        if ((error as NodeJS.ErrnoException).code === "ENOENT") {
          return {
            content: [
              {
                type: "text",
                text: `No attachments folder exists in "${args.folder || "root"}"`,
              },
            ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to list attachments: ${error instanceof Error ? error.message : "Unknown error"}`,
          },
        ],
        isError: true,
      };
...
export const uploadAttachmentTool = defineTool({
  ...
  description:
    "Upload a file attachment to a folder. The file content must be base64 encoded. Files are stored in .attachments subdirectory.",
  inputSchema: z.object({
    folder: z
      .string()
      .default("")
      .describe("Folder path to upload to (empty for root)"),
    filename: z
      .string()
      .min(1)
      .describe('Name for the file (e.g., "diagram.svg", "image.png")'),
    ...
    try {
      ...
      if (
        ...
        return {
          content: [
            {
              type: "text",
              text: 'Invalid filename: cannot contain path separators or ".."',
            },
          ],
          isError: true,
        };
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to upload attachment: ${error instanceof Error ? error.message : "Unknown error"}`,
          },
        ],
        isError: true,
      };
...
export const deleteAttachmentTool = defineTool({
  ...
  inputSchema: z.object({
    folder: z
      .string()
      .default("")
      .describe("Folder path containing the attachment (empty for root)"),
  ...
  handler: async (args, context) => {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        return {
          content: [
            { type: "text", text: `Attachment not found: ${args.filename}` },
          ],
          isError: true,
        };
      }
      return {
        content: [
          {
            type: "text",
            text: `Failed to delete attachment: ${error instanceof Error ? error.message : "Unknown error"}`,
          },
        ],
        isError: true,
      };
...
export const getAttachmentTool = defineTool({
  name: "get_attachment",
  description:
    "Get the content of an attachment file as base64. Useful for reading and transferring files.",
  inputSchema: z.object({
    folder: z
      .string()
      .default("")
    ...
    try {
      ...
      if (mimeType.startsWith("image/")) {
        return {
          content: [
            {
              type: "text",
              text: `Attachment: ${args.filename} (${formatBytes(stats.size)})`,
            },
      ...
      return {
        content: [
          {
            type: "text",
            text: `Failed to get attachment: ${error instanceof Error ? error.message : "Unknown error"}`,
          },
        ],
        isError: true,
      };
