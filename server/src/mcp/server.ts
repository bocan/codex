...
function validateApiKey(req: IncomingMessage): boolean {
  ...
  if (!config.apiKey) {
    if (config.debug) {
      console.log(
        "[MCP] Warning: No API key configured, accepting all requests",
      );
    }
    return true;
  }
...
export function startMcpServer(): void {
  ...
  server.listen(config.port, config.host, () => {
    console.log(
      `🤖 MCP Server started on http://${config.host}:${config.port}`,
    );
    ...
    if (!config.apiKey) {
      console.log(
        "   ⚠️  Warning: No MCP_API_KEY set - server is unauthenticated!",
      );
    }
  });
}
